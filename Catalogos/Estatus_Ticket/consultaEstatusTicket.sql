 ...
 CREATE PROCEDURE consultaEstatusTicket (   IN iIDEstado   INTEGER,
     ...
     /*Manejo de Errores*/ 
		...
		START TRANSACTION;
			...
			IF EXISTS(SELECT * FROM ctEstatusTicket WHERE ctEstatusTicket.iIDEstado = iIDEstado)

				/*Si existe copia toda la informacion del usuario a la tabla temporal*/
				THEN INSERT INTO tt_ctEstatusTicket SELECT * FROM ctEstatusTicket WHERE ctEstatusTicket.iIDEstado = iIDEstado;
			...
			IF NOT EXISTS(SELECT * FROM tt_ctEstatusTicket WHERE tt_ctEstatusTicket.lActivo = 1)
