 ...
 */
 ...
 CREATE PROCEDURE consultaCriticidad(	IN cCriticidad  VARCHAR(20),
 									OUT lError TINYINT(1), 
 									OUT cSqlState VARCHAR(50), 
 									OUT cError VARCHAR(200))
 	consultaCriticidad:BEGIN
		...
		START TRANSACTION;
			...
			IF EXISTS(SELECT * FROM ctCriticidad WHERE ctCriticidad.cCriticidad = cCriticidad)

				/*Si existe copia toda la informacion del edificio a la tabla temporal*/
				THEN INSERT INTO tt_ctCriticidad SELECT * FROM ctCriticidad WHERE ctCriticidad.cCriticidad = cCriticidad;

				/*Si no manda error de que no lo encontro*/
				ELSE 
					SET lError = 1; 
					SET cError = "Criticidad no existe";
					LEAVE consultaCriticidad;

			END IF;

			/*Valida que el edificio este activo*/
			IF NOT EXISTS(SELECT * FROM tt_ctCriticidad WHERE tt_ctCriticidad.lActivo = 1)

				THEN 
					SET lError = 1; 
					SET cError = "Criticidad no activo";
					LEAVE consultaCriticidad;
