 ...
 */
 ...
 CREATE PROCEDURE consultactPersona( IN iPersona INTEGER,
 	...
 	consultactPersona:BEGIN
		...
		START TRANSACTION;
			...
			DROP TEMPORARY TABLE IF EXISTS tt_ctPersona;
 
 			CREATE TEMPORARY TABLE tt_ctPersona LIKE ctPersona;

 			
			/*Comprueba si existe el Tipo Persona*/

			IF EXISTS(SELECT * FROM ctPersona WHERE ctPersona.iPersona = iPersona
												AND ctPersona.iIDTipoPersona  = iIDTipoPersona)

			/*Si existe copia toda la informacion del Tipo Persona a la tabla temporal*/
 				THEN INSERT INTO tt_ctPersona SELECT * FROM ctPersona WHERE ctPersona.iPersona = iPersona
 															AND ctPersona.iIDTipoPersona = iIDTipoPersona;
			...
			/*Si no manda error de que no lo encontro*/
 				ELSE 
 					...
 					LEAVE consultactPersona;	

 			 END IF;

 			SELECT * FROM tt_ctPersona;
