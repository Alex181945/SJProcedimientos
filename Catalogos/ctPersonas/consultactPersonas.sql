 ...
 */
 ...
 CREATE PROCEDURE consultactPersonas(IN iIDTipoPersona INTEGER,
 										OUT lError     TINYINT(1), 
 										OUT cSqlState  VARCHAR(50), 
 										OUT cError     VARCHAR(200))

 	/*Nombre del Procedimiento*/
 	consultactPersona:BEGIN
		...
		START TRANSACTION;
			...
			DROP TEMPORARY TABLE IF EXISTS tt_ctPersonas;
 
 			CREATE TEMPORARY TABLE tt_ctPersonas LIKE ctPersonas;

 			
			/*Comprueba si existe el Tipo Persona*/

			IF EXISTS(SELECT * FROM ctPersonas WHERE ctPersonas.iIDTipoPersona  = iIDTipoPersona)

			/*Si existe copia toda la informacion del Tipo Persona a la tabla temporal*/
 				THEN INSERT INTO tt_ctPersonas SELECT * FROM ctPersonas ctPersonas.iIDTipoPersona = iIDTipoPersona;

			/*Si no manda error de que no lo encontro*/
 				ELSE 
 					SET lError = 1; 
 					SET cError = "Tipo Persona no existe";
 					LEAVE consultactPersonas;	

 			 END IF;

 			SELECT * FROM tt_ctPersonas;
