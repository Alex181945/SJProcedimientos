 ...
 */
 ...
 CREATE PROCEDURE consultaopGrupos( IN iGrupo       INTEGER,
 									IN iCarrera   INTEGER,
 									IN iPeriodo	  INTEGER,
 	...
 	consultaopGrupos:BEGIN
		...
		START TRANSACTION;
			...
			DROP TEMPORARY TABLE IF EXISTS tt_opGrupos;
 
 			CREATE TEMPORARY TABLE tt_opGrupos LIKE opGrupos;

 			
			/*Comprueba si existe el Tipo Persona*/

			IF EXISTS(SELECT * FROM opGrupos WHERE opGrupos.iGrupo  = iGrupo
														AND opGrupos.iCarrera  = iCarrera
														AND opGrupos.iPeriodo  = iPeriodo)

			/*Si existe copia toda la informacion del Tipo Persona a la tabla temporal*/
 				THEN INSERT INTO tt_opGrupos SELECT * FROM opGrupos WHERE opGrupos.iGrupo = iGrupo
 																AND opGrupos.iCarrera  = iCarrera
																AND opGrupos.iPeriodo  = iPeriodo;
