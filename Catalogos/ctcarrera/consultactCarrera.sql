 ...
 */
 ...
 CREATE PROCEDURE consultactCarrera( IN iCarrera INTEGER,
 	...
 	consultactCarrera:BEGIN
		...
		START TRANSACTION;
			...
			IF EXISTS(SELECT * FROM ctCarrera WHERE ctCarrera.iCarrera  = iCarrera
														AND ctCarrera.cCarrera  = cCarrera)

			/*Si existe copia toda la informacion del Tipo Persona a la tabla temporal*/
 				THEN INSERT INTO tt_ctCarrera SELECT * FROM ctCarrera WHERE ctCarrera.iCarrera = iCarrera
 																AND ctCarrera.cCarrera = cCarrera;
			/*Si no manda error de que no lo encontro*/
 				ELSE 
 					SET lError = 1; 
 					SET cError = "Tipo Carrera no existe";
 					LEAVE consultactCarrera;	

 			 END IF;

 			  			/*Valida que el Tipo Persona este activo*/
 			IF NOT EXISTS(SELECT * FROM tt_ctCarrera WHERE tt_ctCarrera.lActivo = 1)
