 ...
 */
 ...
 CREATE PROCEDURE consultaopGrupoDets( IN iGrupo INTEGER,
 										IN iPartida		  INTEGER,
 	...
 	consultaopGrupoDets:BEGIN
		...
		START TRANSACTION;
			...
			DROP TEMPORARY TABLE IF EXISTS tt_opGrupoDets;
 
 			CREATE TEMPORARY TABLE tt_opGrupoDets LIKE opGrupoDets;

 			
			/*Comprueba si existe el Tipo Persona*/

			IF EXISTS(SELECT * FROM opGrupoDets WHERE opGrupoDets.iGrupo  = iGrupo
														AND opGrupoDets.iPartida  = iPartida
														AND opGrupoDets.iTipoPersona  = iTipoPersona
														AND opGrupoDets.iPersona = iPersona
														AND opGrupoDets.iMateria = iMateria)

			/*Si existe copia toda la informacion del Tipo Persona a la tabla temporal*/
 				THEN INSERT INTO tt_opGrupoDets SELECT * FROM opGrupoDets WHERE opGrupoDets.iGrupo = iGrupo
 																AND opGrupoDets.iPartida  = iPartida
																AND opGrupoDets.iTipoPersona  = iTipoPersona
																AND opGrupoDets.iPersona = iPersona
																AND opGrupoDets.iMateria = iMateria;
			...
			/*Si no manda error de que no lo encontro*/
 				ELSE 
 					...
 					LEAVE consultaopGrupoDets;	

 			 END IF;

 			  			/*Valida que el Tipo Persona este activo*/
 			IF NOT EXISTS(SELECT * FROM tt_opGrupoDets WHERE tt_opGrupoDets.lActivo = 1)
